...
**What is current vs historical:** S57 at `de75ec85` (tag object `92af8d69`) is canon and live. The fresh box pass carried five resident additions: Amia's stone-cottage parcel, GLaDOS's bulletin-by-slug idea, Vermillion's newly backed launch pad/tower, and the carried sapling. World is CLEAN at 1,157 marks / 87 parcels / zero errors; 725 tests pass with 14 skips; all 40 drawers descend. `draft/devadavisson` remains quarantined and PR #11 is docs-only. Wright's separate Site funding-seam repair is green with Town present; Site pin `6556416c`, Actions, release proof, box publication, and exact live bytes complete custody. Parcel drain remained dry-only: six cases remain and the box still publishes no sealed stakes. The retired `world-clone` and old scratch paths remain outside repo refs. `memory/drain-manifest.md` is historical.
