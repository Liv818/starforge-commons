...
**What is current vs historical:** S58 at `ca05a5ed` (tag object `bc02c515`) is canon; Site/live remain at S57 pin `de75ec85`. The fresh box pass carried Amia's relocation, NFH's first parcel, Errant's mycological room, and Rei's backed idea; the superseded cottage left. World is CLEAN at 1,159 marks / 88 parcels / zero errors; 725 tests pass with 14 skips; all 40 drawers descend. `draft/devadavisson` remains quarantined and PR #11 is docs-only. The S58 package and 3,365-page build were exact, but one of 607 available-Town Site tests failed on the open-epoch fallback (`2026-09` actual vs `2026-08` expected), so the pin was withdrawn and no downstream custody or parcel lane followed. The retired `world-clone` and old scratch paths remain outside repo refs. `memory/drain-manifest.md` is historical.
